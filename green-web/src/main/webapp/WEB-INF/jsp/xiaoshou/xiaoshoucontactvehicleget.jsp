...
<body>
    <%--合同条款部分--%>
    <div>
        <spring-form:form commandName="execute" id="contact_tiche_form" class="layui-form" action="${pageContext.request.contextPath}/contact/xiaoshoucontactvehicleget.html" method="post">
                     ...
                     <div class="layui-form-item">
                         ...
                         <div class="layui-input-block">
                            <select id="moduleBrand" name="moduleBrand" lay-filter="moduleBrand">
                                ...
                                <c:forEach items="${contact.origModules}" var="loop">
                                    <option value="${loop.moduleBrand}|${loop.leftTiCheNumber}">${loop.moduleName} [${loop.moduleType} - ${loop.moduleBrand}<c:if test="${loop.moduleDianLiang != null && loop.moduleDianLiang != ''}">(${loop.moduleDianLiang}度电)</c:if> - ${loop.moduleColor}]  [合同车辆:${loop.saleNumber}辆 - 已提车${loop.alreadyTiChe}辆]
                                    </option>
                                </c:forEach>
                            </select>
                        </div>
