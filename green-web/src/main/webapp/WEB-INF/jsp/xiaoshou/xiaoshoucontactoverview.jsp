...
<div class="admin-main">
    ...
    <blockquote class="layui-elem-quote">
        <form class="layui-form"  id="visit_search_form"  action="${pageContext.request.contextPath}/contact/xiaoshoucontactoverview.html" method="POST">
            <input type="hidden" name="init" value="true"/>
                ...
                <div class="layui-input-inline">
                    ...
                    <yj:security grantRoles="ROLE_GREEN_CONTACT_SALE,ROLE_GREEN_ADMIN">
                        <a href="${pageContext.request.contextPath}/contact/xiaoshoucontactform.html" class="layui-btn layui-btn-small " id="add">
        ...
        <div class="layui-field-box">
            <table class="site-table table-hover table_box">
                <thead>
                    ...
                    <th class="th_time">合同类型</th>
                    <th class="th_time" >合同电费</th>
                ...
                <c:forEach items="${contacts}" var="line" varStatus="counter">
                    <tr class="gradeX">
                        ...
                        <td>
                            ${line.xiaoShouType.description}
                        </td>
                        <td>
                             ${line.baoDianType.description}
                        </td>
                        <td>
                        ...
                        <td class="center">
                            <c:if test="${line.contactStatus == 'S_CREATED'}">
                                ...
                                <a href="${pageContext.request.contextPath}/contact/xiaoshoucontactform.html?xiaoShouContactUuid=${line.uuid}" class="layui-btn layui-btn-mini layui-btn-normal" style="margin-right:6px;vertical-align:middle;"><i class="layui-icon">&#xe63c;</i>编辑</a>
                            </c:if>
                            <c:if test="${line.contactStatus == 'S_PROCESSING'}">
                            ...
                            <c:if test="${fn:length(line.origModules)>0}">
                                <c:if test="${line.contactStatus != 'S_CREATED'}">
                                    <a href="${pageContext.request.contextPath}/contact/xiaoshoucontactpdfdownload.html?xiaoShouContactUuid=${line.uuid}" class="layui-btn layui-btn-mini layui-btn-warm" style="margin-right:6px;vertical-align:middle;"><i class="layui-icon">&#xe625;</i>下载</a>
                                </c:if>
                                <a href="${pageContext.request.contextPath}/contact/xiaoshoucontactdetails.html?xiaoShouContactUuid=${line.uuid}" class="layui-btn layui-btn-mini layui-btn-normal" style="margin-right:6px;vertical-align:middle;"><i class="layui-icon">&#xe62a;</i>详情</a>
                            </c:if>
            ...
            <div class="admin-table-page">
                <div id="page" class="page">
                    <yj:paging urlMapping="${pageContext.request.contextPath}/contact/xiaoshoucontactoverview.html" paging="${paging}"/>
                </div>
            </div>
    </fieldset>
    </fieldset>
...
<script type="text/javascript">
    ...
    function changeSortBy(anotherSort) {
        ...
        window.location.href = '${pageContext.request.contextPath}/contact/xiaoshoucontactoverview.html?addSort=true&sortBy=' + sortBy + '&sortWay=' + sortWay;
    }

    /*********************************************合同状态部分**************************************************/

    function openContactCheckModal(xiaoShouContactUuid, changeStatus) {
        selectZuPinContactUuid = xiaoShouContactUuid;
        selectChangeStatus = changeStatus;
        layer.confirm('请确认是否开始执行该合同？', {
            btn : [ '确定', '取消' ]//按钮
        }, function(index) {
            layer.close(index);
            window.location.href = '${pageContext.request.contextPath}/contact/xiaoshoucontactcheck.html?xiaoShouContactUuid=' + selectZuPinContactUuid + "&changeStatus=" + selectChangeStatus;
       });
    }
    ...
    function openContactDeleteModal(xiaoShouContactUuid) {
        selectZuPinContactUuid = xiaoShouContactUuid;
        layer.confirm('请确认是否删除该合同？', {
            btn : [ '确定', '取消' ]//按钮
        }, function(index) {
            layer.close(index);
            window.location.href = '${pageContext.request.contextPath}/contact/xiaoshoucontactdelete.html?xiaoShouContactUuid=' + selectZuPinContactUuid;
        });
    }
