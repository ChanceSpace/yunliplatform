...
<div class="layui-row pay_container">
    <yj:security grantRoles="ROLE_GREEN_CONTACT_SALE,ROLE_GREEN_CONTACT_FINANCE">
        ...
        <c:if test="${fn:length(willPayOrder.histories)>0}">
            <div class="no_pay pay_list">
                ...
                <c:forEach items="${willPayOrder.histories}" var="rentHistory">
                    <div class="pay_con" data-price="${rentHistory.feeTotal}" data-id="${rentHistory.uuid}">
                        <div class="pay_con_left">
                            <div class="pay_con_left_left">
                        ...
                        <div class="pay_con_right">
                            <P class="pay_name"> <c:if test="${rentHistory.vehicleNumber!=null&&rentHistory.vehicleNumber!=''}">车牌号：${rentHistory.vehicleNumber}&nbsp;&nbsp;</c:if> 扣费类型：${rentHistory.xiaoShouContactHistoryType.description}</P>
                            <p class="pay_price">
                                <span class="font_fd6636 bold">￥${rentHistory.feeTotal}</span>
                            </p>
                            <p class="pay_remark">
...
<c:if test="${notFinishOrder == null}">
    <footer class="pay-footer">
        <c:if test="${fn:length(willPayOrders)>0}">
            <form action="${pageContext.request.contextPath}/carrier/xiaoshoupaymentfirststep.html" method="post">
...
<script type="text/javascript">
    ...
    layui.use([ 'icheck', 'form'], function() {
        ...
        form.on('select(selectCarrier)', function(data){
            window.location.href = "${pageContext.request.contextPath}/carrier/xiaoshoupaymentmanagement.html?selectCarrierUuid="+data.value;
        });
    ...
    function continueToPay(orderNumber) {
        window.location.href = "${pageContext.request.contextPath}/carrier/xiaoshoupaymentfirststep.html?payOrderNumber=" + orderNumber;
    }
    ...
    function searchPayStatus(payOrderNumber, selectCarrierUuid) {
        layer.confirm('请确认是否要检查支付平台状态？', {
            ...
            layer.close(index);
            window.location.href='${pageContext.request.contextPath}/carrier/xiaoshoupaymentupdate.html?payOrderNumber=' + payOrderNumber + '&selectCarrierUuid=' + selectCarrierUuid;
        });
    }
    ...
    function cancelOrder(payOrderNumber, selectCarrierUuid) {
        layer.confirm('请确认是否取消该订单？', {
            ...
            layer.close(index);
            window.location.href='${pageContext.request.contextPath}/carrier/xiaoshoupaymentcancel.html?payOrderNumber=' + payOrderNumber + '&selectCarrierUuid=' + selectCarrierUuid;
        });
    }
    ...
    function  cancelPayHistory(historyUuid) {
        layer.confirm('请确认是否取消该订单？', {
            ...
            layer.close(index);
            settings.source ='${pageContext.request.contextPath}/carrier/xiaoshoucancelHistory.html?historyUuid=' + historyUuid+'&selectCarrierUuid='+'${selectCarrierUuid}';
