package com.yajun.green.web.controller.xiaoshoupay;
...
import com.yajun.green.service.*;
import com.yajun.green.web.controller.pay.OnlinePaymentHandler;
import com.yajun.green.web.controller.pay.OnlinePaymentResult;
import com.yajun.green.web.controller.pay.PayOrderManagementController;
 ...
 */
@Controller
public class XiaoShouPaymentManagementController {
    @Autowired
    ...
    @Autowired
    private XiaoShouContactPayService xiaoShouContactPayService;
    @Autowired
    private XiaoShouPaymentService xiaoShouPaymentService;

    /*******************************************未支付订单和款项查询模块******************************************/

    @RequestMapping("/carrier/xiaoshoupaymentmanagement.html")
    public String payOrderManagement(HttpServletRequest request, HttpServletResponse response, ModelMap model){
        ...
        if (SecurityUtils.hasSaleAccess()) {
            //通过renthistory 查询并去重
            List<CompanyDTO> companies = xiaoShouContactPayService.obtainAllCompanyWithOnCreateBillOnLoginUser();
        ...
        //查询所有source 类型为 销售合同类型的 未完成payorder
        PayOrderDTO notFinishOrder = payOrderService.obtainNotFinishXiaoShouOrderByChengYunUuid(selectCarrierUuid);
        model.put("notFinishOrder", notFinishOrder);

        //获取所有特定承运商 销售合同未完成renthistory
        List<ZuPinXiaoShouPayOrderManagementShowDTO> willPayOrders = xiaoShouContactPayService.obtainAllOnCreateBill(selectCarrierUuid);
        //去除已经有的在订单中的
        if (notFinishOrder != null) {
            for (ZuPinXiaoShouPayOrderManagementShowDTO history : willPayOrders) {
                history.removeAlreadyTakenPayment(notFinishOrder.getPayItems());
            }
        }
        model.put("willPayOrders", willPayOrders);

        return "xiaoshoupay/xiaoshoupaymentmanagement";
    }
    ...
    //http://localhost:8080/my/carrier/xiaoshoupayorderfirststep.html?totalFee=30000.00&chengYunUuid=201701010101_1234567890123404&itemUuids=20171017_0858_3109813946996242,20171017_0858_6006739345578845,20171017_0858_7303854724935709,20171017_0858_7332500417819751
    @RequestMapping("/carrier/xiaoshoupaymentfirststep.html")
    public String payOrderFirstStep(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (StringUtils.hasText(payOrderNumber)) {
            PayOrderDTO payOrder = payOrderService.obtainPayOrder(payOrderNumber, false, false, false,true);
        ...
        return "xiaoshoupay/xiaoshoupaymentfirststep";
    }

    /******************************************线下支付***************************************************/

    @RequestMapping("/carrier/xiaoshoupaymentoffline.html")
    public String payOrderOffline(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (PayOrderWay.valueOf(orderWay).equals(PayOrderWay.O_OFFLINE) && chengYun.getCurrentBalance().doubleValue() < totalFee) {
            return "redirect:xiaoshoupaymentmoneynotenought.html";
        }
        ...
        payOrderNumber = xiaoShouPaymentService.updateBeginXiaoShouContactPayOrderOffline(payOrderNumber, jiaoYiNumber, chengYunUuid, actorUuid, PayOrderSource.O_XS, PayOrderWay.valueOf(orderWay), orderNote, BigDecimal.valueOf(totalFee), items, fujianFiles);

        return "redirect:xiaoshoupaymentfinish.html?payOrderNumber=" + payOrderNumber + "&totalFee=" + totalFee;
    }

    /******************************************线上支付***************************************************/

    @RequestMapping("/carrier/xiaoshoupaymentonlinefirst.html")
    public String payOrderOnlineFirst(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        String[] temp = StringUtils.delimitedListToStringArray(itemUuids, ",");
        List<String> items = Arrays.asList(temp);
        PayOrderDTO payOrder = xiaoShouPaymentService.updateBeginXiaoShouContactPayOrderOnline(payOrderNumber, chengYunUuid, actorUuid, PayOrderSource.O_XS, PayOrderWay.valueOf(orderWay), BigDecimal.valueOf(totalFee), items);
        ...
        if (ApplicationEventPublisher.applicationPaymentLive) {
            String payURL = OnlinePaymentHandler.handleOnlinePaymentURL(payOrder, false);
            if (StringUtils.hasText(payURL)) {
                return "redirect:" + payURL;
            }
            return "redirect:xiaoshoupaymentonlinefinishwaiting.html";
        }

        return "redirect:xiaoshoupaymentwaiting.html?payOrderNumber=" + payOrderNumber;
    }

    @RequestMapping("/carrier/xiaoshoupaymentwaiting.html")
    public String payOrderWaiting(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        return "xiaoshoupay/xiaoshoupaymentwaiting";
    }

    @RequestMapping("/carrier/xiaoshoupaymentonlinesecond.html")
    public void payOrderOnlineChecking(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (result.getResult() == 0) {
            BigDecimal totalFee = xiaoShouPaymentService.updateFinishContactPayOrderOnline(payOrderNumber, result.getJiaoYiNumber());
            total.put("FLAG", "YES");
    ...
    @RequestMapping("/carrier/xiaoshoupaymentonlinesecondfake.html")
    public String payOrderOnlineSecondFake(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        BigDecimal totalFee = xiaoShouPaymentService.updateFinishContactPayOrderOnline(payOrderNumber, jiaoYiNumber);
        return "redirect:xiaoshoupaymentfinish.html?payOrderNumber=" + payOrderNumber + "&totalFee=" + totalFee;
    }

    /*********************************************支付结果页面****************************************************/

    @RequestMapping("/carrier/xiaoshoupaymentmoneynotenought.html")
    public String payOrderMoneyNotEnough(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        return "xiaoshoupay/xiaoshoupaymentmoneynotenough";
    }

    @RequestMapping("/carrier/xiaoshoupaymentonlinefinishwaiting.html")
    public String payOrderFinishWaiting(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        return "xiaoshoupay/xiaoshoupaymentonlinefinishwaiting";
    }

    @RequestMapping("/carrier/xiaoshoupaymenterror.html")
    public String payOrderError(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        return "xiaoshoupay/xiaoshoupaymentonlineerror";
    }

    @RequestMapping("/carrier/xiaoshoupaymentfinish.html")
    public String payOrderFinish(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        model.put("payOrderNumber", payOrderNumber);
        model.put("totalFee", totalFee);
        return "xiaoshoupay/xiaoshoupaymentfinish";
    }

    /*********************************************取消订单****************************************************/

    @RequestMapping("/carrier/xiaoshoupaymentcancel.html")
    public String payOrderCancel(HttpServletRequest request, HttpServletResponse response, ModelMap model) throws Exception {
        ...
        if (canDelete) {
            return "redirect:xiaoshoupaymentmanagement.html?selectCarrierUuid=" + selectCarrierUuid;
        }
        model.put("payOrderNumber", payOrderNumber);
        return "xiaoshoupay/xiaoshoupaymentalreadyfinish";
    }

    /*********************************************手动更新订单状态************************************************/

    @RequestMapping("/carrier/xiaoshoupaymentupdate.html")
