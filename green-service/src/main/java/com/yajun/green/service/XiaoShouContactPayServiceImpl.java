 ...
 */
@Service("xiaoShouContactPayService")
public class XiaoShouContactPayServiceImpl implements XiaoShouContactPayService {

    @Autowired
    ZuPinContactDao zuPinContactDao;
    @Autowired
    XiaoShouContactDao xiaoShouContactDao;

    /*********************************合同账单相关*******************************/
    @Override
    public List<ZuPinXiaoShouPayOrderManagementShowDTO> obtainAllOnCreateBill(String selectCarrierUuid) {
        //获取特定用户所有未付款账单
        List<XiaoShouContactRentingFeeHistory> histories = xiaoShouContactDao.findSpecUserOnCreateBill(selectCarrierUuid);
        Map<String, ZuPinXiaoShouPayOrderManagementShowDTO> orderMap = new HashMap();

        if (histories != null) {
            for (XiaoShouContactRentingFeeHistory history : histories) {
                XiaoShouContact contact = history.getXiaoShouContact();
                ...
                ZuPinXiaoShouPayOrderManagementShowDTO show = orderMap.get(contactNumber);

                if (show == null) {
                    show = new ZuPinXiaoShouPayOrderManagementShowDTO(jiaFangName, yiFangName, contactNumber);
                }
                ...
                ZuPinXiaoShouContactRentingFeeHistoryDTO historyDTO = XiaoShouContactRentingFeeHistoryWebAssember.toRentFeeHistoryDTO(history);
    ...
    public List<CompanyDTO> obtainAllCompanyWithOnCreateBillOnLoginUser() {
        List<Object[]> users = xiaoShouContactDao.obtainAllCompanyWithOnCreateBillOnLoginUser();
    ...
    @Override
    public void updateXiaoShouHasPay(Set<String> xiaoShouContactUuids) {
        String[] contactUuids = (String[]) xiaoShouContactUuids.toArray(new String[xiaoShouContactUuids.size()]);
